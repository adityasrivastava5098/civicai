...
const ComplaintTable = ({ complaints, onUpdateStatus }) => {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-sm">
        ...
        <div className="flex items-center gap-3">
          <div className="relative group">
            ...
            <input 
              ...
              className="bg-slate-950 border border-slate-800 rounded-2xl py-2 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500/50 w-full md:w-64 transition-all"
              value={searchTerm}
          ...
          <select 
            className="bg-slate-950 border border-slate-800 rounded-2xl py-2 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500/50 transition-all appearance-none cursor-pointer pr-10"
              ...
              <tr key={item.id} className="hover:bg-slate-800/30 transition-colors group">
                <td className="px-6 py-4">
                  <Link 
                    to={`/issue/${item.id}`} 
                    target="_blank" 
                    className="text-blue-400 hover:text-blue-300 font-bold decoration-blue-400/30 underline-offset-4 hover:underline"
                  >
                    {item.id}
                  </Link>
                </td>
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <select 
                      ...
                      className="bg-slate-950 border border-slate-700 rounded-xl px-2 py-1 text-xs focus:ring-1 focus:ring-blue-500 outline-none cursor-pointer hover:border-blue-500/50 transition-all opacity-0 group-hover:opacity-100 focus:opacity-100"
                    >
                      <option value="Open">Open</option>
                      <option value="In Progress">In Progress</option>
                      <option value="Resolved">Resolved</option>
      ...
      <div className="p-4 bg-slate-950/30 text-xs text-slate-500 flex justify-between items-center px-6 border-t border-slate-800">
        <p>Showing {filteredComplaints.length} of {complaints.length} records</p>
        <div className="flex gap-2">
           <button className="px-3 py-1 bg-slate-800 rounded-lg hover:bg-slate-700 disabled:opacity-50">Prev</button>
